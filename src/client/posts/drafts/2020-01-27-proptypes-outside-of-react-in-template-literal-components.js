...
I recently moved away from using JSX as my templating system and switched to using JavaScript’s template literals. One of the things I didn’t want to lose when switching was the functionality of the [`prop-types`](https://www.npmjs.com/package/prop-types) package.
...
1. The raw data from my content matched my expectations in my templates
2. Any transformations on the raw content from my data was transformed correctly and met my expectations.
...
What’s really cool about the `prop-types` package is that you can use it outside of React/JSX. Let me give you an example.

Let’s say I had a React/JSX component and I wanted to check some props:
...
How do I maintain continue to use `prop-types` when I’m not using React/JSX, since React is the one that does the prop checking for you?

My template literal “components” looked very similar to my JSX components: they are both functions that take data and return something. JSX returns a JavaScript object meant for consumption by React. My template files return a tagged template literal that evaluates to a string. Combine this with the fact that the `prop-types` package provides a [`checkPropTypes` function](https://github.com/facebook/prop-types#proptypescheckproptypes) so that you can use the library outside of React, and you’ve got a simple recipe for calling prop-types: call it in the template function itself. Example:
...
When that template file gets run executed in node, it checks the prop types and returns any relevant warnings. Beautiful!
...
```js
import { checkPropTypes, string } from "prop-types";
import { withPropTypeChecks } from "./utilities.js";
