...
In the third iteration, our biggest breakthrough was a simplification of how to choose an insurance product. Rather than make the user submit the form *and then* return a list of products, we realized we should just do that interaction in real-time. We also figured it would be nice to have more graphical, “marketing”-type respresentations of each product.

For the initial go around I mocked this out without a lot of thought into what the graphical representation of the product would be. I just used big colors so we could wire it up to an Invision mock and click around and see if it felt right. The process was mostly the same in the sense that the user still have to enter an address first. If they didn’t have an address, we would fall back to letting them choose the state. Then based on the “smart” defaults (or whatever the user changed them to) the UI would dynmically draw the list of available products.

![Third iteration of initial step for starting a new quote]({{ site.imageurl }}/2017/qqs-v3-step-1.png "Initial screen where you enter an address")

![Third iteration of address autosuggest step for starting a new quote]({{ site.imageurl }}/2017/qqs-v3-step-2.png "Autosuggest of addresses lets you quickly find an address")

![Third iteration of choosing a product when starting a new quote]({{ site.imageurl }}/2017/qqs-v3-step-3.png "Dynamic list of products populates on the right as values on the left are changed")
...
![First version of the fourth iteration for product selection when starting a new quote]({{ site.imageurl }}/2017/qqs-v4-iteration-1.png)

That didn’t quite feel right, so I tried redrawing the insurnace products as smaller thumbnails.

![Second version of the fourth iteration for product selection when starting a new quote]({{ site.imageurl }}/2017/qqs-v4-iteration-2.png)
...
![Third version of the fourth iteration for product selection when starting a new quote]({{ site.imageurl }}/2017/qqs-v4-iteration-3.png)
...
![Final version of the fourth iteration for product selection when starting a new quote]({{ site.imageurl }}/2017/qqs-v4-iteration-4.png)
...
![Final mock for when no addresses could be found when starting a new quote]({{ site.imageurl }}/2017/qqs-final-no-auto-suggestions.png "When no address could be found through Google’s API, we provided a shortcut to switching to a U.S. state input.")

![Final mock for when no products could be found when starting a new quote]({{ site.imageurl }}/2017/qqs-final-no-products.png "When no products were available, we asked for feedback so we could better discover what our clients were needing.")

![Final mock for when no products could be found when starting a new quote]({{ site.imageurl }}/2017/qqs-final-no-accessible-products.png "When no products were accessible, we asked for further feedback along with the option to get in contact with the client to help them find what they needed.")

Oh, and in case you’re curious, there was the initial screen that the user would see when no data had been entered yet.

![Final mock for the initial zero state when starting a new quote]({{ site.imageurl }}/2017/qqs-final-initial.png)
...
After release, the problem we discovered wass that the addresses being entered by some users were not showing up in the autocomplete. We *thought* we had properly accounted for this edge case during the design phase by providing an action in the case of zero auto-suggested addresses. This action allowed the user to continue getting a quote by entering the state of the property rather than the address. However, we discovered a whole in our process which looked like this:

1. Hit Google’s place autocomplete API, which allows you to enter a natural lannguage address and get back the component parts of an address (verified through Google), i.e. "760 E 3rd Ave Salt Lake City" will give you back "address: 760 E 3rd Ave; city: Salt Lake City; state: UT; zip: 84103" broken into their respective pieces for ease of handling from a programatic perspective.
...
Initially I think our product team thought we were going to have to add a couple more forms to the entire screen just to accomodate for this edge case, but that felt like a lot of overkill. The view was already large enough and we wanted to keept the perceived complexity of the form UI to a mimimum. So I sketched through a couple ideas of how to incorporate this action all within the address field.

![Photo of sketch for UI ideas on solving address field edge case bug]({{ site.imageurl }}/2017/qqs-address-fix-drawing.jpg "Sketch of ideas on incorporating handling address search edge cases inside the address lookup field.")

I mocked out a few screens through Invision that walked through how this process would work in the UI, which ended uplooking like this:

![Animated gif of the address edge case UI as a mock]({{ site.imageurl }}/2017/qqs-address-fix-flow-mock.gif)

When this actually got wired up, it looked something like this:

![Animated gif of the address edge case UI as a live example]({{ site.imageurl }}/2017/qqs-address-fix-flow-live.gif)
